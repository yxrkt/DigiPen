...
void AngleBetweenLinePlane()
{
	...
	double cos1 = Dot(V,W)/(V.length()*W.length());

	AssertEq(cos0*cos0 + cos1*cos1, 1.0, "Angle between line and plane.");
}
...
void TrianglePointContainment()
{
	...
	Point3D In = A + 0.4*(B-A) + 0.3*(C-A);
	Point3D Out = A + 0.6*(B-A) + 0.7*(C-A);
	Point3D Off = In + Cross(B-A, C-A);
	...
	if (TRI.contains(Off)) {
		AssertNotHere("Triangle3D/Point3D Point off plane."); }
	else {
		AssertHere; }
}
